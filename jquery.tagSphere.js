(function(){
    ...
    function initTags(tagCont){
        tags2 = tagCont.children('ul').children();
        tags2.css({
            'position': 'absolute',
        ...
        // tag should aim for the el that we want to display in the cloud
        //console.log(tagCont.children('ul').children('li').children('a').children());
        tags = tagCont.children('ul').children('li').children('a').children();
        tags.css({
            'position': 'absolute',
            'list-style-type': 'none',
            'list-style-position': 'outside',
        ...
        if (Math.abs(fy) > 0.01 || Math.abs(fx) > 0.01) {
            ...
            while (j++ < forCalcs.tLength) {
                ...
                tags[j].css({
                    ...
                    'height': options.fontMultiplier * tags[j].alpha*1.5 + 'px',
                    'width' : options.fontMultiplier * tags[j].alpha*1.5 + 'px',
